...
class Produto {
    ...
    public Produto(String nome, String categoria, int quantidade, int minimo, double preco, String unidade) {
        this.id = (int) contadorId.incrementAndGet();
        this.nome = nome;
    ...
    public String toString() {
        return String.format("[%02d] %-25s | %-12s | Qtd: %3d %-3s | Min: %2d | R$ %6.2f | %s", id, nome, categoria,
                quantidade, unidade, minimo, preco, getStatus());
    }
}
...
class Movimentacao {
    ...
    @Override
    public String toString() {
        return String.format("[%s] %-8s | %-25s | Qtd: %3d | %s | Obs: %s", dataHora, tipo, produto, quantidade,
                tipo.equals("ENTRADA") ? "▲" : "▼", obs.isEmpty() ? "-" : obs);
    }
}
...
class EstoqueService {
    ...
    public void adicionarProduto(String nome, String categoria, int quantidade, int minimo, double preco,
            String unidade) {
    ...
    public boolean editarProduto(int id, String nome, String categoria, int quantidade, int minimo, double preco,
            String unidade) {
    ...
    public void ajustarEstoque(int id, String tipo, int quantidade, String obs) {
        ...
        System.out.printf("  ✓ Estoque ajustado. Novo saldo: %d %s%n", p.getQuantidade(), p.getUnidade());
    }
    ...
    public List<Produto> filtrarPorCategoria(String categoria) {
        ...
        return produtos.stream().filter(p -> p.getCategoria().equalsIgnoreCase(categoria)).collect(Collectors.toList());
    }

    public List<Produto> buscarPorNome(String termo) {
        return produtos.stream().filter(p -> p.getNome().toLowerCase().contains(termo.toLowerCase()))
                .collect(Collectors.toList());
    }

    public List<Produto> getAlertas() {
        return produtos.stream().filter(p -> !p.getStatus().equals("NORMAL")).collect(Collectors.toList());
    }
    ...
    public void exibirProdutos(List<Produto> lista) {
        ...
        System.out.println("\n" + "─".repeat(85));
        System.out.printf("%-4s %-25s %-13s %-10s %-6s %-10s %s%n", "ID", "Nome", "Categoria", "Qtd", "Min", "Preço",
                "Status");
...
public class EstoqueMercadinho {
    ...
    static class Menu {
        ...
        private void editarProduto() {
            ...
            servico.editarProduto(id, nome.isBlank() ? p.getNome() : nome,
                    categoria.isBlank() ? p.getCategoria() : categoria, qtd == 0 ? p.getQuantidade() : qtd,
                    minimo == 0 ? p.getMinimo() : minimo, preco == 0 ? p.getPreco() : preco,
        ...
        private void excluirProduto() {
            System.out.println("\n── EXCLUIR PRODUTO ──");
            int id = lerInt("ID do produto: ");
            Optional<Produto> opt = Optional.ofNullable(null);
            if (opt.isEmpty()) {
                System.out.println("  ✗ Produto não encontrado.");
                return;
            }
        ...
        private void exibirAlertas() {
            ...
            System.out.println("\n── ALERTAS ──");
            if (alertas.isEmpty())
                System.out.println("  ✓ Nenhum produto com problema de estoque.");
            else
                servico.exibirProdutos(alertas);
        }
